...
#endif
#if HAVE_LIBLZ4
  LZ4F_errorCode_t result =  LZ4F_createDecompressionContext( &DATA(io)->dcCtxt, LZ4F_VERSION );
  ...
  while(true) {
    ...
    if(data_size < 256*1024) {
      if(data_size == 0) {
        DATA(io)->inbuf_index = 0;
        DATA(io)->inbuf_len = 0;
      } else if((sizeof(DATA(io)->inbuf) - DATA(io)->inbuf_len) < 256*1024) { /* compact, only if buffer became smallish */
        memmove(DATA(io)->inbuf, DATA(io)->inbuf + DATA(io)->inbuf_index, data_size);

        DATA(io)->inbuf_index = 0;
        DATA(io)->inbuf_len = data_size;
      }
      while(true) {
        /* Decompressing large buffers optimize zstd and lz4, */
        /* by largely avoiding copy into internal temp buffers */
        int bytes_read = wandio_read(DATA(io)->parent,
                                     DATA(io)->inbuf + DATA(io)->inbuf_len,
                                     sizeof(DATA(io)->inbuf) - DATA(io)->inbuf_len);
        ...
        if ((bytes_read == 0) && (DATA(io)->inbuf_len == DATA(io)->inbuf_index)) {
          DATA(io)->err = ERR_EOF;
          return outbuf_index; /* EOF here too*/
        }
        DATA(io)->inbuf_len += bytes_read;
        if(bytes_read == 0 || DATA(io)->inbuf_len >= (int64_t)sizeof(DATA(io)->inbuf)) {
          break;
        }
      }
    }
    ...
    while(true) {
      if(DATA(io)->dec == DEC_UNDEF) {
        if(len < 4) {
          fprintf(stderr, "Invalid too short ZSTD/LJS frame\n");
          errno = EIO;
          return -1;
        }
        unsigned char* buf = (unsigned char*) (DATA(io)->inbuf + DATA(io)->inbuf_index);
        if (((buf[0] & 0xf0) == 0x50) && (buf[1] == 0x2a) &&
...
#endif
#if HAVE_LIBLZ4
        } else if ((buf[0] == 0x04) && (buf[1] == 0x22) &&
                   (buf[2] == 0x4d) && (buf[3] == 0x18)) {
          DATA(io)->dec = DEC_LZ4;
#endif
        } else {
          fprintf(stderr, "Unknown ZSTD/LJS frame \n");
          DATA(io)->dec = DEC_UNDEF;
          errno = EIO;
          return -1;
        }
      ...
      if(DATA(io)->dec == DEC_UNDEF) {
        ...
        /* dec == DEC_SKIP_FRAME added twice below in endif because HAVE_LIBZSTD can be false */
#if HAVE_LIBZSTD
      } else if (DATA(io)->dec == DEC_ZSTD || DATA(io)->dec == DEC_SKIP_FRAME) {
        DATA(io)->input_buffer.src = DATA(io)->inbuf;
        DATA(io)->input_buffer.pos = DATA(io)->inbuf_index;
        DATA(io)->input_buffer.size = DATA(io)->inbuf_len;
        DATA(io)->output_buffer.dst = buffer;
        DATA(io)->output_buffer.pos = outbuf_index;
        DATA(io)->output_buffer.size = len;

        size_t result = ZSTD_decompressStream(DATA(io)->stream,
        ...
        DATA(io)->inbuf_index = DATA(io)->input_buffer.pos;
        if (result == 0) { /* we finished frame */
          DATA(io)->dec = DEC_UNDEF;
        }
#endif
#if HAVE_LIBLZ4
      } else if (DATA(io)->dec == DEC_LZ4 || DATA(io)->dec == DEC_SKIP_FRAME) {
        size_t src_ptr = DATA(io)->inbuf_len - DATA(io)->inbuf_index;
        size_t dst_ptr = len - outbuf_index;
        LZ4F_errorCode_t result = LZ4F_decompress(DATA(io)->dcCtxt, buffer + outbuf_index, &dst_ptr,
                                                  DATA(io)->inbuf + DATA(io)->inbuf_index, &src_ptr, NULL);
        if (LZ4F_isError(result)) {
          fprintf(stderr, "lz4 decompress error %ld %s\n", result, LZ4F_getErrorName(result));
          DATA(io)->err = ERR_ERROR;
          errno = EIO;
          return -1;
        }
      ...
      if(DATA(io)->inbuf_index == inbuf_index_save) {
        fprintf(stderr, "zstd - lz4 decoder does not advance reads");
        errno = EIO;
        DATA(io)->err = ERR_ERROR;
        errno = EIO;
        return -1;
