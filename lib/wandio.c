...
DLLEXPORT iow_t *wandio_wcreate(const char *filename, int compress_type, int compression_level, int flags) {
	...
	/* This is very wrong - zlib (gzip) is around 5 times slower on compression than zstd,
	 * and zstd provides slightly better compression than zlib.
	 * zstd should be preferable */
#if HAVE_LIBZ
...
#endif
#if HAVE_LIBLZO2
	else if (compress_type == WANDIO_COMPRESS_LZO) {
		iow = lzo_wopen(iow, compression_level);
	}
#endif
#if HAVE_LIBBZ2
	else if (compress_type == WANDIO_COMPRESS_BZ2) {
		iow = bz_wopen(iow, compression_level);
	}
#endif
#if HAVE_LIBLZMA
	else if (compress_type == WANDIO_COMPRESS_LZMA) {
		iow = lzma_wopen(iow, compression_level);
	}
#endif
#if HAVE_LIBZSTD
	else if (compress_type == WANDIO_COMPRESS_ZSTD) {
		iow = zstd_wopen(iow, compression_level);
	}
#endif
#if HAVE_LIBLZ4
	else if (compress_type == WANDIO_COMPRESS_LZ4) {
		iow = lz4_wopen(iow, compression_level);
	}
#endif
