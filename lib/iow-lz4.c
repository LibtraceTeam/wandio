...
iow_t *lz4_wopen(iow_t *child, int compress_level) {
  ...
  LZ4F_errorCode_t result =  LZ4F_createCompressionContext( &DATA(iow)->cctx, LZ4F_VERSION );
  ...
  result = LZ4F_compressBegin( DATA(iow)->cctx, DATA(iow)->outbuf, sizeof(DATA(iow)->outbuf),
          DATA(iow)->prefsPtr);
  if (LZ4F_isError(result)) {
...
static int64_t lz4_wwrite(iow_t *iow, const char *buffer, int64_t len){
  ...
  /* We need to handle arbitrarily large input buffer, by limiting to 1MB pieces */
  int inbuf_len;       /* piece len */
  int inbuf_index = 0; /* index is from begin of buffer, can spawn several pieces */
  while(true) {
    ...
    size_t upper_bound = LZ4F_compressBound(inbuf_len, DATA(iow)->prefsPtr);
    if ((size_t)upper_bound > sizeof(DATA(iow)->outbuf) - DATA(iow)->outbuf_index) {
      int bytes_written = wandio_wwrite(DATA(iow)->child, DATA(iow)->outbuf, DATA(iow)->outbuf_index);
      if (bytes_written <= 0) {
    ...
    size_t result = LZ4F_compressUpdate(
            DATA(iow)->cctx, DATA(iow)->outbuf + DATA(iow)->outbuf_index,
            sizeof(DATA(iow)->outbuf) - DATA(iow)->outbuf_index, buffer + inbuf_index, inbuf_len, NULL);
    if (LZ4F_isError(result)) {
...
static int lz4_wflush(iow_t *iow) {
  int64_t bytes_written = wandio_wwrite(DATA(iow)->child, DATA(iow)->outbuf, DATA(iow)->outbuf_index);
  if (bytes_written < 0) {
  ...
  assert(bytes_written == DATA(iow)->outbuf_index);
  DATA(iow)->outbuf_index = 0;
  size_t result = LZ4F_flush(DATA(iow)->cctx, DATA(iow)->outbuf, sizeof(DATA(iow)->outbuf), NULL);
  if (LZ4F_isError(result)) {
  ...
  if(result > 0) {
    bytes_written = wandio_wwrite(DATA(iow)->child, DATA(iow)->outbuf, result);
    if (bytes_written <= 0) {
      fprintf(stderr, "lz4 compress flush error\n");
      DATA(iow)->err = ERR_ERROR;
      return -1;
...
static void lz4_wclose(iow_t *iow) {
  lz4_wflush(iow);
  size_t result = LZ4F_compressEnd(DATA(iow)->cctx, DATA(iow)->outbuf, sizeof(DATA(iow)->outbuf), NULL);
  if (LZ4F_isError(result)) {
    fprintf(stderr, "lz4 compress close error %ld %s\n", result, LZ4F_getErrorName(result));
    errno = EIO;
  }
  int64_t bytes_written = wandio_wwrite(DATA(iow)->child, DATA(iow)->outbuf, result);
  if (bytes_written <= 0) {
